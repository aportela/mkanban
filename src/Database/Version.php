<?php
      ...
      class Version {
        ...
        public function __construct (\mKanban\Database\DB $dbh, string $databaseType) {
            $this->dbh = $dbh;
        ...
        public function get() {
            ...
            if ($results && count($results) == 1) {
                ...
                throw new \mKanban\Exception\NotFoundException("invalid database version");
            }
        }

        private function set(float $number) {
            $params = array(
                (new \mKanban\Database\DBParam())->str(":num", (string) $number)
            );
            $query = '
                INSERT INTO VERSION
                    (num, date)
